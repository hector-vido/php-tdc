...
-- ------------------------------------------------------------------------
-- This is an example
-- We create a InnoDB table `session` is the `tdc` database
-- and insert an entry into containers' table to tell InnoDB Memcache
-- that we has such InnoDB table as back store.
-- PRIMARY -> use primary key to search
-- ------------------------------------------------------------------------

INSERT INTO containers VALUES ("php", "tdc", "session",
                               "id", "value",  "flag", "cas", "expire", "PRIMARY");
...
-- ------------------------------------------------------------------------
-- Key (id) must be VARCHAR or CHAR type, memcached supports key up to 255
-- Bytes
-- Value (value) must be VARCHAR or CHAR type
-- Flag (flag) is a 32 bits integer
-- CAS (cas) compare and swap is a 64 bits integer, per memcached define
-- Exp (expire) is again a 32 bits integer
-- ------------------------------------------------------------------------

DROP TABLE IF EXISTS session;
CREATE TABLE session (id VARCHAR(32),
                        value VARCHAR(1024),
                        flag INT, cas BIGINT UNSIGNED, expire INT, primary key(id))
ENGINE = INNODB;
